...
<div style="flex: 50%;max-width: 50%;padding: 0 4px;" class="column">
    ...
    <div class="form-group row ">
      ...
      <!-- Phone Field -->
    <div class="form-group row ">        {!! Form::label('phone', trans("lang.user_phone"), ['class' => 'col-3 control-label text-right']) !!}
        <div class="col-9">            {!! Form::text('phone', null,  ['class' => 'form-control','placeholder'=>  trans("lang.user_phone_placeholder")]) !!}
...
<?php if(in_array("driver", $rolesSelected) && $user->is_active == '1'){?>
    <div class="form-group row ">
        {!! Form::label('restaurants[]', trans("lang.food_restaurant_id"),['class' => 'col-3 control-label text-right']) !!}
        <div class="col-9">

            {!! Form::select('restaurants[]', ['all' => 'Select All',"clear" => 'Clear All']+$restaurants, $restaurantSelected, ['class' => 'select2 form-control res_select', 'multiple'=>'multiple']) !!}
...
$(document).ready(function(){
    ...
    $('.res_select').on("select2:select", function (e) { 

        var data = e.params.data.text;

        if(data == 'Select All'){
            $(".res_select > option").prop("selected","selected");
            $('.res_select').find("option[value='all']").prop('selected',false);
            $('.res_select').find("option[value='clear']").prop('selected',false);
            $(".res_select").trigger("change");      
        }
        if(data == 'Clear All'){
            $(".res_select").val(null).trigger("change");
        }      
    });
});
</script>
