...
class OrderController extends Controller
{
    ...
    public function __construct(OrderRepository $orderRepo, CustomFieldRepository $customFieldRepo, UserRepository $userRepo
        , OrderStatusRepository $orderStatusRepo, NotificationRepository $notificationRepo, PaymentRepository $paymentRepo,FoodOrderRepository $foodOrderRepository,
    ...
    public function store(CreateOrderRequest $request)
    {
        ...
        $delivery_address['user_id'] = $input['user_id'];
        ...
        $this->orderRepository->update(['delivery_address_id'=>$address_id->id],$order->id);        
    ...
    public function show(FoodOrderDataTable $foodOrderDataTable, $id)
    {
        ...
        $restaurant_name = Order::join("food_orders", "orders.id", "=", "food_orders.order_id")
                ->join("foods", "foods.id", "=", "food_orders.food_id")
                ->join("restaurants", "restaurants.id", "=", "foods.restaurant_id")
                ->where('orders.id',$id)
                ->groupBy('orders.id')
                ->select('restaurants.name as res_name') 
                ->get()->pluck('res_name')->toArray();
        ...
        return $foodOrderDataTable->render('orders.show', ["order" => $order, "total" => $total, "subtotal" => $subtotal,"restaurant_name" => $restaurant_name]);
    }
