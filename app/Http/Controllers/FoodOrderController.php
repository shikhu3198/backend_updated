...
private $orderRepository;
    ...
    /** 
    * update order.json 
    * it is set new order to 0
    * @return true on success
    */
    public function updateOrderJson(Request $request) 
    {
        try{
            $str = file_get_contents('order.json');
            $json = json_decode($str); 
        } catch(\ErrorException $e) {
            $json = (object) array("is_new_order"=>1);
            
        }       
        if(!empty($json)) { 
            $json->is_new_order = 0;
            $updatedData  =  json_encode($json);
            try {
                file_put_contents("order.json",$updatedData);
            } catch(\ErrorException $e) {
                return Response::json(["file don't have permission to write"],403);        
            }       
        }
