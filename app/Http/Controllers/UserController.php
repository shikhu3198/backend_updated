...
class UserController extends Controller
{
    ...
    public function profile()
    {
        ...
        $Selectedrestaurant = \DB::table('driver_restaurants')
        ...
        if(count($Selectedrestaurant) > 0)
        {
            foreach ($Selectedrestaurant as $key => $value) {
                $ids[] = $value->restaurant_id;
            }

            $restaurantSelected = Restaurant::whereIn('id',$ids)->pluck('id','id')->toArray();
        }
        else
        {
            $restaurantSelected = [] ;  
        ...
        return view('settings.users.profile', compact(['user', 'role', 'rolesSelected', 'customFields', 'customFieldsValues','restaurants','restaurantSelected']));
    }
    ...
    public function create()
    {
        ...
        return view('settings.users.create')
            ...
            ->with("restaurantSelected", $restaurantSelected)
    ...
    public function update($id, UpdateUserRequest $request)
    {
        ...
        try {
            ...
            if(isset($input['restaurants']) && !empty($input['restaurants']))
            {
                $getDriverData = \DB::table('driver_restaurants')->where('user_id',$id)->get()->toArray();
                
                $final_ar = array();
                foreach ($getDriverData as $key => $value) {
                    array_push($final_ar, $value->restaurant_id);
                    
                }
                $count = 0;
                    foreach ($input['restaurants'] as $dkey => $dvalue) 
                    {                       
                        $insert_data = array(
                            'user_id' => $id,
                            'restaurant_id' => $dvalue
                        );

                        if($count == 0)
                        {
                            \DB::delete('delete from driver_restaurants where user_id = ?',[$id]);
                            $count++;                
                        }
                        \DB::table('driver_restaurants')->insert($insert_data);
                    }
